...
export default function FixedBillsWidget({
  data,
  loading,
  month,
  year,
}: FixedBillsWidgetProps) {
  const t = useTranslations("fixed-bills");
  const locale = useLocale();
  const loc = locale === "pt-br" ? "pt-BR" : "en-US";

  const monthName = new Date(year, month - 1).toLocaleDateString(loc, {
    month: "long",
  });
          ...
          {loading || !data ? (
            ...
            <>
              ...
              >
                {[
                  {
                    label: t("summary.estimated"),
                    value: fmt(data.totalEstimated),
                  },
              ...
              <Stack spacing={0.75} sx={{ mb: 2 }}>
                {data.pendingOccurrences.slice(0, 3).map((occ) => {
                  ...
                  const dayStr = dueDate.toLocaleDateString(loc, {
                    day: "2-digit",
                    month: "2-digit",
                  });
                  return (
